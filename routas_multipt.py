...
for i in range(0,len(pt)):
	...
	ini_node = ox.get_nearest_node(graphs[0], ini_xy, method = 'euclidean') ##extrai o nó do OSM mais próximo do ponto    
  
	fim =  pt[i+1]['geometry'] #extrai as informações do novo ponto final
	fim = Point(fim.x,fim.y)  
	fim_xy = (fim.y, fim.x)
	fim_node = ox.get_nearest_node(graphs[0], fim_xy, method = 'euclidean')
